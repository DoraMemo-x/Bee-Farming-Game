...
int honeyKg = 0;
boolean gameScreenActive = true, shopScreenActive = false, sellShopScreenActive = false, 
  ...
  if (gameOver) {
    ...
    if (GTShopScreenActive) {
      ...
      if (GTOngoing) {
        ...
        text("Check the Guardian Training tab.", 50, height-100);
      } else if (GTPurchased) {
        text("You have purchased a " + upgradedGuardianName[GTSelected] + " training course.", 50, height-125);
        text("The course will take place after this week.", 50, height-100);
      }
...
void topStatsBar() {
  ...
  fill(105);
  textSize(9);
  text("Ver 17.07.05 <Pre-Alpha>", width-20, 65); //keyword: version, ver.
...
void mouseReleased() {
  ...
  if (gameScreenActive) {
    ...
    if (GTPurchase.MouseClicked()) {
      ...
      //adding objectives in Forest.pde
    }
  }
