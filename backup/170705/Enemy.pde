...
class Hornet {
  ...
  void showHornet() {
    ...
    stroke(0); 
    strokeWeight(1.5);
    if (killFatigue) ellipse(location.x, location.y, radius*(5000-(gameMillis-lastKillMillis))/1500, radius*(5000-(gameMillis-lastKillMillis))/1500);
...
class Firefly {
  ...
  void showFirefly() {
    ...
    textSize(10);
    textAlign(CENTER);
    if (targetable) text("Targetable", location.x, location.y-15);
    else text(fireflyTap + (fireflyTap > 1 ? " taps" : " tap"), location.x, location.y-15);
  }
