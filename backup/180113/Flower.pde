...
void createFlower() {
  ...
  for (Bee b : bees) {
    ...
    if (determVal > 0) factBeeAtt += determVal;
    else {
      if (determVal < -0.2) factBeeAttDeduct += determVal*1;
      else if (determVal < -0.4) factBeeAttDeduct += determVal*0.5;
      else factBeeAttDeduct += determVal*0.6;
  ...
  if (x < 6.773) nextFlowerTime = 0.0799*pow(x, 1.9871)*1000;
  else nextFlowerTime = (1.2915*x-5.1714)*1000;

  if (factFlowerN <= bees.size()) { // if there are no flowers
