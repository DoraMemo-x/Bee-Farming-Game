...
void beeTrainMechanicRun(boolean showText) {  
  ...
  if (roundTime >= ROUND_TIME) { //mostly failed. but if the training course has "tier system", then it might not be.
...
class Plot {
  ...
  void showPlot() {
    ...
    case 0:
      ...
      chargeBar(location.x-plotSize*0.4, location.y-plotSize*0.65, plotSize*0.8, 15, color(#f5deb3), progressTimer, BTGrowthTime[plantType], false, true, false);
