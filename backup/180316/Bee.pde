...
int[] upgradeBeeCost = {70000, 55000, 90000, 70000}; //money
String[][] upgradedBeeDescription = {
  {"|b"+upgradedBeeName[0], "|i$"+nfc(int(upgradeBeeCost[0])) + " (Training Needed)", "", "|rAttributes:", "- Unable to gather honey.", "- Moves incredibly slowly.", "", "|rAbility "+ char(34) + "" + char(1505)+""+char(1492)+""+char(1503)+""+char(1499)+char(1492)+""+char(1501)+""+char(1496)+""+char(1506)+""+char(1495)+""+char(1506)+""+char(1502)+""+char(1508)+""+char(1499)+"" + char(34)+":", "- Revives A Dead Bee", "  * Reviving time depends on the tier."}, //1498=A 
  {"|b"+upgradedBeeName[1], "|i$"+nfc(int(upgradeBeeCost[1])) + " (No Training)", "", "|rAttribute: Has two lives.", "- First Life:", "  Moves very quickly but gathers honey slowly", "- Second Life:", "  Moves significantly slower but gathers honey much quicker;", "  However, its capacity becomes smaller"}, 
  {"|b"+upgradedBeeName[2], "|i$"+nfc(int(upgradeBeeCost[2])) + " (Training Needed)", "", "|rResource Ability " + char(34) + "Nature's Path" + char(34) + ":", "  Stores 20% of its gathered honey,", "  Charges a toggleable ability for", "  planting flowers in the path it moves in.", "", "  * Honey amount depends on the tier.", "Drawback:", "- Mediocre flight speed"}, 
  {"|b"+upgradedBeeName[3], "|i$"+nfc(int(upgradeBeeCost[3])) + " (No Training)", "", "|rCooldown Ability " + char(34) + "Adrenaline" + char(34) + ":", "  Increases its flight speed", "  and honey pick-up speed for the duration.", "", "Drawback:", "- Normally mediocre honey pick-up speed"}, 
};
...
class Bee {
    ...
    if (isAlive && forShop == false) {
      ...
      case 0: //Priest Bee
        if (beeTarget.equals("Revive")) {
          ...
          circularTimer(location.x, location.y, diameter*3, diameter*3, CDATimer, maxDuration, color(242, 195, 74, 150), false, false, true);
          //noStroke();
          //fill(255, 255, 0, 150);
  ...
  void move() {
    ...
    if (isAlive) {
      speed = beeFlightSpeed[beeType] * (60/frameRate); //speed correction
      ...
      if (beeType >= beeName.length) { //if it is a special bee (trained bee)
        ...
        case 0: //Priest Bee
          ...
          if (CDAbility) { //if using ability
            if (beeSelectedStatus == false && roundTime < ROUND_TIME) {
              shouldBeeMove = false;
            ...
            } else if (roundTime >= ROUND_TIME) {
              //still reviving when round's over. reset to cancel revive & make bee go towards beehive
              resetRevive();
  ...
  void dumpHoneyAnimation() {
    ...
    if (roundOver) {
      if (diameter > 0) diameter -= 0.5; 
        ...
        beeTarget = "None";
        int addHoneyKg = floor((currHoneyCap)/1000);
        honeyKg += addHoneyKg;
        honeyKg = constrain(honeyKg, 0, honeyPotCapKg[honeyPotTier]);
        if (honeyKg + addHoneyKg > honeyPotCapKg[honeyPotTier]) {
          totalHoneyCollected += honeyPotCapKg[honeyPotTier]-honeyKg;
          honeyOverflown += honeyKg+addHoneyKg-honeyPotCapKg[honeyPotTier];
        } else totalHoneyCollected += addHoneyKg;
...
class TrainingBee {
  ...
  void move() {
    ...
    if (trainingType == 2) speed = trainingBeeFlightSpeed[2]*1.25*(60/frameRate); //speed correction due to frame drops
