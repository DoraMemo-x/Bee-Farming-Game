...
static final String[] upgradedGuardianName = {"Royal Guardian", "Baiting Guardian", "Hunting Guardian", "Border Guardian", "Ranged Guardian", "Bouncer Guardian"};
float[][] upgradeGuardianCost = {{75000, 1}, {65000, 1}, {100000, 1}, {9999999, 99}, {75000, 1}, {70000, 0.5}}; //money, week
String[][] upgradedGuardianDescription = {
  ...
  {"|b"+upgradedGuardianName[2], "|i$"+nfc(int(upgradeGuardianCost[2][0])) + ", " + nf(upgradeGuardianCost[2][1]) + (upgradeGuardianCost[2][1] > 1 ? " weeks" : " week"), "", "|rCooldown Ability " + char(34) + "Hunter's Aura" + char(34) + ":", "- Enlarge an AOE area gradually, fully extending it after 1.5s", "- Slows down enemies in its enlarged area", "- When active, the guardian itself cannot move.", "", "Drawback:", "  Normally extremely small sensing area", "  with a mediocre flight speed."}, 
  {"|b"+upgradedGuardianName[3], "|i$"+nfc(int(upgradeGuardianCost[3][0])) + ", " + nf(upgradeGuardianCost[3][1]) + (upgradeGuardianCost[3][1] > 1 ? " weeks" : " week"), "", "|rCooldown Ability " + char(34) + "Bound" + char(34) + ":", "- Throws a special object that bounces.", "- The object can trap enemies in it, or even killing it.", "- Activate the object by clicking on the guardian.", "  This will stablize the object and cause it to magnify.", "", "Drawback:", "- The guardian itself cannot kill enemies."}, 
...
color[] guardianColour = {color(186, 186, 255), color(136, 136, 255), color(86, 86, 255), beeColour[0], color(124, 70, 10), color(255, 196, 196), color(113, 100, 86), color(0)};
...
class Guardian {
  ...
  Guardian(String _name) {
    ...
    if (guardianType == 5) stingAmount = 1; //border
    else if (guardianType == 6) stingAmount = 2; //ranged
  }
  ...
  void showGuardian() {
    ...
    } else if (guardianType == 5) {
      if (hyperSelectedStatus) {
        fill(255, 196, 196);
        textSize(10);
        textAlign(CENTER);
        text("Bound ready to shoot!", location.x, location.y-15);
      } else if (CDAbility) {
        boolean show = false;
        for (ProjectileWeapon pw : projectiles) {
          if (pw.getID() == distinctID && pw.getBeforeActivate()) {
            show = true;
            break;
          }
        }
        if (show) {
          ...
          textSize(10);
          textAlign(CENTER);
          text("Press me to activate.", location.x, location.y-15);
        }
      }
    ...
    if (shouldGuardianMove) { //if the guardian is in forest (i.e. not as inv display / minimap)
      ...
      } else if (guardianType == 5 && stingAmount < 1 && CDAbility == false) { //border guardian
        strokeWeight(1);
        stroke(50);
        noFill();
        rect(location.x+12, location.y-10, 6, 20);
    ...
    if (speedBuffTier >= 1) {
      ...
      pushMatrix();
      translate(radius/GUARDIAN_RADIUS*15, 0);
      rotate(HALF_PI);
      text("§", 0, 0);
      popMatrix();
    ...
    if (speedBuffTier >= 2) {
      ...
      pushMatrix();
      translate(-radius/GUARDIAN_RADIUS*sqrt(2)/2*15, -radius/GUARDIAN_RADIUS*sqrt(2)/2*15);
      rotate(radians(-30));
      text("§", 0, 0);
      popMatrix();
...
boolean shooting = false;
  void move() {
    speed = (guardianFlightSpeed[guardianType]+0.325*speedBuffTier) * (60/frameRate); //speed correction
    ...
    if (guardianTarget.equals("Firefly") == false) {
      if (demo) {
        ...
        if (closestHornetID != -1) {
          ...
          float hypotenuse = distance_between_two_points(location.x, hx, location.y, hy);
          if (hypotenuse < 1.5) {
            if (h.getHornetType() == 1) addEnemyElimScore(0);
            else if (h.getHornetType() == 2) addEnemyElimScore(1);
            hornets.remove(h);
            questEnemyProg++;
            if (guardianType == 7) {
              targetBeeID = -1;
    ...
    if (guardianType == 3 && roundOver == false) { //baiting guardian mechanic 
      ...
      if (CDAInitiateTimer >= 500) CDAbility = true; //held for 0.5s, activate
        ...
        CDACooldown = 8000;
        shouldGuardianMove = true;
        speed = (guardianFlightSpeed[guardianType]+0.325*speedBuffTier) * (60/frameRate);
      ...
      for (Hornet h : hornets) {
        ...
        if (hypotenuse <= constrain(map(CDATimer, 0, 1500, guardianSenseArea[guardianType], 300), guardianSenseArea[guardianType], 300)) {
          ...
          h.updateSpeedDebuff(true);
        } else h.updateSpeedDebuff(false);
      }
      ...
      for (Firefly ff : fireflies) {
        ...
        if (hypotenuse <= constrain(map(CDATimer, 0, 1500, guardianSenseArea[guardianType], 300), guardianSenseArea[guardianType], 300)) {
          ff.updateSpeedDebuff(true);
        } else ff.updateSpeedDebuff(false);
      }
    } else if (guardianType == 5) { //computer -> border
    shooting = false;
      if (hyperSelectedStatus) {
        if (mousePressed) {
          float shootTheta = calcMoveTheta(mouseX, mouseY, location.x, location.y);
          projectiles.add(new ProjectileWeapon(2, -1, location.x, location.y, shootTheta, false));
          projectiles.get(projectiles.size()-1).updateID(distinctID);
          stingAmount--;
          hyperSelectedStatus = false;
          CDAbility = true;
      ...
      if (stingAmount == 0 && CDAbility == false) { //defined "when object is in forest = using ability"
        CDACooldown -= int(timeMillis-pMillis);
        CDACooldown = constrain(CDACooldown, 0, 8000);
      }
    ...
    shooting = false;
      ...
      if (guardianTarget.equals("Hornet") == false && guardianTarget.equals("Firefly") == false /* and all other enemy targets */) { //not chasing enemy
              ...
              if (b.getBeeSelectedStatus()) {
                ...
                if (targetBeeID != i) {
                  ...
                  if (targetBeeID != -1 && CDAbility) { //the guardian previously had a PROTECTING target 
                    ...
                    beeDeaths++; //consider removing? since it's not an enemy kill
                    CDAbility = false;
                    targetBeeID = i;
                  } else targetBeeID = i;
        ...
        if (guardianTarget.equals("Bee") && 
          ...
          shouldGuardianMove = true;
          speed = (guardianFlightSpeed[guardianType]+0.325*speedBuffTier) * (60/frameRate); //return to normal speed
  ...
  int getSpeedBuffTier() {
    return speedBuffTier;
  }
}
