...
int[] beeCost = {300, 1000, 5000, 10000, 15000, 50000 /*, not for sale*/};
int[] beeSellCost = {beeCost[0]/2, beeCost[1]/2, beeCost[2]/2, beeCost[3]/2, beeCost[4]/2, beeCost[5]/2, /*100000*/};
int[] guardianCost = {8000, 18000 /*, not for sale*/};
int[] guardianSellCost = {guardianCost[0]/2, guardianCost[1]/2};
...
int[] honeyPotPrice = {0, 1000, 3500, 12000, 30000, 120000, 800000};
//static final int[] honeyPotSellPrice = {0, 500, 1250, 5000, 12500, 50000, 250000};

int beehiveTier = 0;
static final int[] beehiveSize = {5, 10, 15, 20, 25};
int[] beehivePrice = {0, 3000, 5000, 8000, 9999999};
...
void shopSetup() {
  ...
  for (int i = 1; i < beehiveSize.length-1; i++) { //-1 because extra hive size is now a thing, which is not purchasable
...
void honeyPriceFlutuate() {
  if (gameMillis - honeyPriceChgTimer > newHPCI) {
    ...
    if (effectTakingPlace[0][0] > 0) honeyPriceDet *= random(1.25, 1.5);
...
void showMarket() {
  ...
  for (int i = 0; i < beehiveSize.length-2; i++) { //extra hive size now a thing. -1 and -1
    buyHive.get(i).Draw();
...
void buyBee(int beeType) {
  ...
  if (beesCount < beehiveSize[beehiveTier]) {
    if (money >= beeCost[beeType]) {
      ...
      noti.add(new Notification(12, color(200, 0, 0), 20, height-60, "You don't have enough money!", 2000, "Down Corner"));
      //ERRORnotEnoughMoney = true;
    }
  } else {
    noti.add(new Notification(12, color(200, 0, 0), 20, height-60, "Your beehive is full!", 2000, "Down Corner"));
    //ERRORbeehiveFull = true;
  }
}
...
void buyPot(int potType) {
  ...
  if (potType > honeyPotTier) {
    ...
    noti.add(new Notification(12, color(200, 0, 0), 20, height-60, "Your honey pot is larger than that!", 2000, "Down Corner"));
    //ERRORpotTierLow = true;
  }
}
...
void buyHive(int hiveType) {
  ...
  if (hiveType > beehiveTier) {
    ...
    noti.add(new Notification(12, color(200, 0, 0), 20, height-60, "Your beehive is larger than that!", 2000, "Down Corner"));
    //ERRORhiveTierLow = true;
  }
}
...
void sellBee(int beeNumber) {
  if (beesCount == beehiveSize[beehiveTier]) tact_SellEntity += 1;
