...
void inventorySetup() {
  ...
  //quests
  //confirmSelection = new Button("Confirm", 20 + (blockSize+10)*2.4, 70 + (blockSize+10)*3.55, 120, 35, 251, 200, 55, 119, 94, 26, false, "default");
}
  ...
  for (int y = 0; y < 2; y++) {
    for (int x = 0; x < 5; x++) {
      ...
      if (counter+beeInvSlot < inventoryBees.size() && mouseX > 20 + (blockSize+10)*x && mouseX < 20 + (blockSize+10)*x + blockSize && mouseY > 70 + (blockSize+10)*y && mouseY < 70 + (blockSize+10)*y + blockSize) {
        int beeType = inventoryBees.get(counter+beeInvSlot).getBeeType()-beeName.length; //the array only covers upgraded bees
        if (beeType >= 0) { //if it is a special bee
          String[][] newDescription = upgradedBeeDescription;
          //removing the price & week length text
          String[] frontSet = subset(newDescription[beeType], 0, 1);
          String[] endSet = subset(newDescription[beeType], 2, newDescription[beeType].length-2);
          String[] concatedSet = concat(frontSet, endSet);
          drawPopupWindow(concatedSet, 0, 12, mouseX, mouseY, true, 0, width);
        } else { //else, it's shop bees
          String[] description = {beeName[beeType+beeName.length]}; //+length back to get the normal bee's name
...
void guardianInventoryActive() {
  ...
  if (guardianInvSelected != -1) {
    ...
    int speedBuffTier = invG.getSpeedBuffTier();
    fill(255, 255, 200);
    if (speedBuffTier == 1) text("This guardian seem to move faster..." + speedBuffTier, 20 + (blockSize+10)*2.1, 70 + (blockSize+10)*0.18+16*8);
    else if (speedBuffTier == 2) text("This guardian seem to move quite fast..?", 20 + (blockSize+10)*2.1, 70 + (blockSize+10)*0.18+16*8);
    else if (speedBuffTier >= 3) text("This guardian seem to move very fast!", 20 + (blockSize+10)*2.1, 70 + (blockSize+10)*0.18+16*8);
...
void itemInventoryActive() {
  ...
  for (int i = inventoryBtn.size()-1; i >= 0; i--) { 
    Button btn = inventoryBtn.get(i);
    if (i == 2) btn.updateGreyOut(true); //2 is item inventory
    else btn.updateGreyOut(false);
  }
  inventoryBtn.get(invBtn).Draw();
  GTMarketBtn.updateGreyOut(false);
  ...
  for (int y = 0; y < 2; y++) {
    for (int x = 0; x < 4; x++) {
      ...
      if (counter < inventoryResources.size()) {
        if (rewardSelection[0] && rewardSelectedID == counter) {
          fill(150, 255, 150);
        }
      }
      //block
