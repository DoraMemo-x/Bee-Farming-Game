...
class Hornet {
  ...
  boolean debuffFromHunting = false;
  ...
  void updateSpeedDebuff(boolean _d) {
    debuffFromHunting = _d;
  }

  boolean stopTrackingBee = false; //this changes to true when enemy touches the trapping guardian barrier
  void updateStopTrackingBee(boolean b) {
    stopTrackingBee = b;
  }
  ...
  void move() {
    //speed = (stopTrackingBee?0.5:hornetFlightSpeed[hornetType]) * (debuffFromHunting?0.5:1) * (60/frameRate); //speed correction
    speed = baseSpeed * (60/frameRate); //speed correction

    if (forTraining == false) {
      enemyAliveTime += timePassed;

      if (gameMillis - lastKillMillis < 5000) killFatigue = true;
      else killFatigue = false;

      if (stopTrackingBee == false) {
        ...
        for (int b = bees.size ()-1; b >= 0; b--) {
          ...
          float hypotenuse = dist(location.x, location.y, bx, by);
  ...
  if (forestType >= 6) {
    ...
    if (gameMillis - hornetTimer > nextHornetTime) {
      ...
      if (enemyTypeDet > 4) {
        ...
        if (effectTakingPlace[2][0] == 0) {
          Firefly ff = fireflies.get(fireflies.size()-1);
          ff.updateSpeed(ff.getSpeed()*effectTakingPlace[2][1]);
        }
        hornetTimer = gameMillis;
      } else {
        hornets.add(new Hornet(enemyTypeDet > 1 ? 1 : 0, false));
        if (effectTakingPlace[2][0] == 0) {
          Hornet h = hornets.get(hornets.size()-1);
          h.updateSpeed(h.getSpeed()*effectTakingPlace[2][1]);
        }
        hornetTimer = gameMillis;
      }
    }
...
class Firefly {
  ...
  void updateMoveTheta(float mt) {
    moveTheta = mt;
  }

  float limitTrackingBee = width;
  void updateLimitTrackingBee(float limit) {
    limitTrackingBee = limit;
  }
  
  void updateBorderLimit(float x, float y, float r) {
    borderLoc = new PVector(x, y);
    borderRadius = r;
  }
  ...
  PVector borderLoc = new PVector(width/2, height/2);
  void move() {
    speed = 1.6 * (debuffFromHunting?0.5:1) * (60/frameRate); //speed correction
    ...
    if (forTraining == false) {
      ...
      for (int b = bees.size ()-1; b >= 0; b--) {
        ...
        if (hypotenuse < shortestDistance && dist(borderLoc.x, borderLoc.y, bx, by) < limitTrackingBee) {
          shortestDistance = hypotenuse;
          closestBeeID = b;
        }
      }
      ...
      String exception = "";
      float hypotenuse = dist(location.x, location.y, borderLoc.x, borderLoc.y);
      if (limitTrackingBee < width && hypotenuse > borderRadius-radius) {
        //ArrayList<String> data = new ArrayList<String>();

        bouncing = true;
        exception = "";

        PVector impactPoint = new PVector(location.x+(hypotenuse-(borderRadius-radius))*cos(moveTheta), location.y+(hypotenuse-(borderRadius-radius))*sin(moveTheta));
        ...
        float m = 1/((location.y-borderLoc.y)/(location.x-borderLoc.x));
        //data.add(str(m));
        //stroke(0, 255, 0);
        ...
        float _moveX =   (killFatigue ? 0.7 : 1) * speed*cos(moveTheta);
        float _moveY = - (killFatigue ? 0.7 : 1) * speed*sin(moveTheta);

        PVector a = new PVector(-_moveX, -_moveY);
        PVector b = new PVector(impactPoint.x-borderLoc.x, impactPoint.y-borderLoc.y);
        a.normalize();
        b.normalize();
        ...
        if (m > 0) {
          ...
          if (abs(moveThetaD - newMoveThetaD) < 0.5) { //(+), EXCEPTION: PIERCES THROUGH
          ...
          float detX = borderLoc.x - location.x; //center - loc
          float detY = borderLoc.y - location.y; //center - loc
          //strokeWeight(5);
          //point(impactPoint.x, impactPoint.y);
          ...
          if (detX < 0 && detY > 0) { // - +, quad I
            if (moveThetaD > 90 && moveThetaD <= 180) { // 90 - 180 (included 180), -2i
              ...
              float redLineAngleD = degrees( abs(atan2( (impactPoint.y-borderLoc.y), (impactPoint.x-borderLoc.x) )) ) + 180; //+180, opposite dir
            ...
            if (moveThetaD > 270 && moveThetaD <= 360) { // 270 - 360 (included 360), -2i
              ...
              //determine which direction is the line coming from
              //"dis is the direction when u stand at the point of collision looking out in the direction of the normal btw."
              float redLineAngleD = 180 - degrees( abs(atan2( (impactPoint.y-borderLoc.y), (impactPoint.x-borderLoc.x) )) ); //atan2 calculation is weird. 180 - instead.
        ...
        float moveX =   (killFatigue ? 0.6 : 1) * 3*speed*cos(moveTheta);
        float moveY = - (killFatigue ? 0.6 : 1) * 3*speed*sin(moveTheta);
        
        location = new PVector(location.x+moveX, location.y+moveY); //the actual moving code
        return; //escape the rest
      }
    } else { //forTraining = true
      if (demo) { //GTdemo
