...
static final float[][] upgradeGuardianCost = {{100000, 1}, {80000, 1}, {125000, 1}, {150000, 2}, {100000, 1}, {75000, 0.5}}; //money, week
...
class Guardian {
  ...
  void showGuardian() {
    ...
    else if (guardianType == 6) fill(113, 100, 86); //ranged
    else if (guardianType == 7) fill(0); //bouncer
    stroke(0);
    strokeWeight(2);
    pushMatrix();
    ...
    if (guardianSelectedStatus) {
      ...
      fill(beeColour[bees.get(targetBeeID[0]).getBeeType()]);
      ...
      noStroke();
      fill(0);
      switch (bees.get(targetBeeID[0]).getBeeType()) {
      case 0:
      case 3:
        // *** center bar line ***
        beginShape();
    ...
    if (guardianType == 4 && CDAbility == false) { //hunting guardian
      ...
      //text("Ready for HUNTER'S AURA!", location.x, location.y-15);
  ...
  void move() {
    ...
    if (guardianType == 3) { //baiting guardian mechanic 
      ...
      if (targetBeeID[0] == bees.size()) targetBeeID[0]--;

      if (guardianTarget.equals("Hornet") == false && guardianTarget.equals("Firefly") == false /* and all other enemy targets */) { 
          ...
          if (beesSelectedCurr > 0) {
            for (int i = bees.size()-1; i >= 0; i--) {
              ...
              if (b.getBeeSelectedStatus()) {
                ...
                if (targetBeeID[0] != i) {
                  guardianTarget = "Bee";
                  if (targetBeeID[0] != -1) targetBeeID[1] = i; //the guardian previously had a target 
                  else targetBeeID[0] = i;
                } else { //selected the target that it is protecting
                  hyperSelectedStatus = false;
                }
                break;
              }
        ...
        if (CDAbility && bees.size() > 0) {
          if (guardianTarget.equals("Bee")) {
            ...
            if (targetBeeDistinctID == b.getDistinctID()) { //check if the bee is still the same bee (changes if the bee is sold / killed)
              speed = b.getSpeed(); //update speed to match the current bee's speed so that it doesnt "wiggle"
