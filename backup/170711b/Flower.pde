...
class Flower { 
  ...
  void showFlower() {
    if (forTraining == false) flowerSize = map(honeyGram/1000, forestHoneyRng[forestType][0], forestHoneyRng[forestType][1]+1, 10, 20) * constrain(float(int(gameMillis-100-spawnAnimationTimer))/500, 0, 1);
    ...
    if (forTraining == false) text(ceil(honeyGram/1000)+"kg", f.x, f.y-20); //used to be f.y-8
