...
static final int[] guardianSellCost = {guardianCost[0]/2, guardianCost[1]/2, 20000};

int honeyPriceChgTimer = 0, honeyPriceChgTimes = 0;
//int newHPCI = round(random(30000, 45000)); //HoneyPriceChgInterval
int newHPCI = 3000; //HoneyPriceChgInterval
...
void honeyPriceFlutuate() {
  if (gameMillis - honeyPriceChgTimer > newHPCI) {
    ...
    else newHPCI = round(random(30000, 45000));
    ...
    else if (honeyPriceDet > 0.925) honeyPrice = 100;
    else if (honeyPriceDet >= 0.825) honeyPrice = 90;
    else honeyPrice = floor(map(honeyPriceDet, 0.25, 0.825, 3, 8)) * 10;
...
void showMarket() {
  ...
  for (int i = 0; i < beeName.length; i++) {
    if (i==beeName.length-1) buyBee.get(i).updateGreyOut(true);
    buyBee.get(i).Draw();

    fill(80);
    if (i<=beeName.length-4) {
      if (beesCountIndividual[i] > 0) text("("+beesCountIndividual[i]+")", 50+130*i + 100/2, 60);
    } else {
      if (beesCountIndividual[i] > 0) text("("+beesCountIndividual[i]+")", 110+130*(i-(beeName.length-3)) + 100/2, 115);
    }
  }
  fill(0);
  textAlign(CENTER);
  for (int i = 0; i < beeCost.length; i++) {
    if (i<=beeCost.length-3) text("$"+nfc(beeCost[i]), 50+130*i + 100/2, 95);
    else text("$"+nfc(beeCost[i]), 110+130*(i-(beeCost.length-2)) + 100/2, 150);
  }
  ...
  for (int i = 0; i < guardianName.length; i++) {
    if (i==guardianName.length-1) buyGuardian.get(i).updateGreyOut(true);
    buyGuardian.get(i).Draw();

    fill(80);
    if (guardiansCountIndividual[i] > 0) text("("+guardiansCountIndividual[i]+")", 50+130*i + 100/2, 180);
  }
  fill(0);
  //textAlign(CENTER);
  for (int i = 0; i < guardianCost.length; i++) {
    text("$"+nfc(guardianCost[i]), 50+130*i + 100/2, 150+60);
  }
...
void showSellMarket() {
  ...
  for (int i = 0; i < beeSellCost.length; i++) {
    if (i<=beeCost.length-3)text("$"+nfc(beeSellCost[i]), 50+130*i + 100/2, 95);
    else text("$"+nfc(beeSellCost[i]), 110+130*(i-(beeSellCost.length-3)) + 100/2, 150);
  }
  ...
  for (int i = 0; i < guardianSellCost.length; i++) {
    text("$"+nfc(guardianSellCost[i]), 50+130*i + 100/2, 150+60);
  }
...
void buyBee(int beeType) {
  ...
  if (beesCount < beehiveSize[beehiveTier]) {
    if (money >= beeCost[beeType]) {
      ...
      bees.add(new Bee(width/2, height/2, BEE_RADIUS, beeName[beeType]));
      //beesCount++;
    } else {
