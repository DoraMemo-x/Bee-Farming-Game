...
static final String VERSION = "Ver 18.01.24 <Alpha>";
boolean debug = false;
...
void setup() {
  ...
  loadBtn = new Button("Load", width-150, 250, 100, 35, 251, 200, 55, 119, 94, 26, false, "default"); //forest location: width-150, 250
      ...
      if (mouseX > gx-25 && mouseX < gx+25 && mouseY > gy-25 && mouseY < gy+25) {
        ...
        if (g.getGuardianSelectedStatus() == false) {
          if (g.getGuardianName().equals(upgradedGuardianName[5]) && g.getHyperSelectedStatus()) {
            g.updateHyperSelectedStatus(false); //de-activate the hyper status
