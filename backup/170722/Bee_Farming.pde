...
ArrayList<Firefly> trainingFireflies = new ArrayList<Firefly>(); //for royal guardian, hunting guardian
ArrayList<Hornet> trainingHornets = new ArrayList<Hornet>(); //for hunting guardian
ArrayList<Ladybug> trainingLadybugs = new ArrayList<Ladybug>();
ArrayList<Flower> trainingFlowers = new ArrayList<Flower>(); //for baiting guardian
...
static final String VERSION = "Ver 17.07.22 <Pre-Alpha>";
boolean debug = false;
...
void setup() {
  ...
  startTutorial = new Button("Tutorial", width/2-75, height/2+220, 150, 40, 251, 200, 55, 119, 94, 26);

  //tutorial
  nextPrompt = new Button("NEXT", width/2-75, height/2+220, 150, 40, 251, 200, 55, 119, 94, 26);
  retryTutorial = new Button("Retry", width-170, height/2+220, 150, 40, 251, 200, 55, 119, 94, 26);
  ...
  for (int i = 0; i < beeName.length; i++) {
    if (i < 3) buyBee.add(new Button(beeName[i], 50+195*i, 50, 100, 50, (int)red(beeColour[i]), (int)green(beeColour[i]), (int)blue(beeColour[i]), 100, 85, 0));
    else buyBee.add(new Button(beeName[i], 50+195*(i-3), 105, 100, 50, (int)red(beeColour[i]), (int)green(beeColour[i]), (int)blue(beeColour[i]), 100, 85, 10));
  }

  buyGuardian.add(new Button(guardianName[0], 50+185*0, 170, 100, 50, 186, 186, 255, 70, 70, 155));
  buyGuardian.add(new Button(guardianName[1], 50+185*1, 170, 120, 50, 136, 136, 255, 33, 34, 121));
  ...
  //Sell Market
  for (int i = 0; i < beeName.length; i++) {
    if (i < 3) sellBee.add(new Button(beeName[i], 50+195*i, 50, 100, 50, (int)red(beeColour[i]), (int)green(beeColour[i]), (int)blue(beeColour[i]), 100, 85, 0));
    else sellBee.add(new Button(beeName[i], 50+195*(i-3), 105, 100, 50, (int)red(beeColour[i]), (int)green(beeColour[i]), (int)blue(beeColour[i]), 100, 85, 10));
  }

  sellGuardian.add(new Button(guardianName[0], 50+185*0, 170, 100, 50, 186, 186, 255, 70, 70, 155));
  sellGuardian.add(new Button(guardianName[1], 50+185*1, 170, 120, 50, 136, 136, 255, 33, 34, 121));
  ...
  for (int i = 0; i < guardianName.length; i++) {
    ...
    //showcaseGuardians.get(i).updateMoveTheta(HALF_PI);
  ...
  if (gameOver) {
    ...
    if (gameScreenActive) {
      ...
      for (int h = hornets.size ()-1; h >= 0; h--) {
        ...
        if ((hx > width || hx < 0 || hy > height-40 || hy < 25) && bees.size() == 0) hornets.remove(h);
      }
      ...
      if (mouseX > gx-25 && mouseX < gx+25 && mouseY > gy-25 && mouseY < gy+25) {
        ...
        if (g.getGuardianSelectedStatus() == false) {
          ...
          if (guardsSelectedCurr == 0) { 
            //if the deselected guardian is HUNTING GUARDIAN / RANGED GUARDIAN, 
            //and after the deselect, no guardians have been selected (== 0)
            //(that means the player only selected HUNTING GUARDIAN / RANGED GUARDIAN in first place)
