...
void showMarket() {
  ...
  for (int i = 0; i < beeName.length; i++) {
    ...
    if (i<=beeName.length-3) {
      ...
      if (beesCountIndividual[i] > 0) text("("+beesCountIndividual[i]+")", 110+130*(i-(beeName.length-2)) + 100/2, 115);
    }
  }
  fill(0);
  textAlign(CENTER);
...
void showSellMarket() {
  ...
  for (int i = 0; i < beeSellCost.length; i++) {
    ...
    else text("$"+nfc(beeSellCost[i]), 110+130*(i-(beeSellCost.length-2)) + 100/2, 150);
  }
