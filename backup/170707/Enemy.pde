  ...
  if (forestType >= 6) {
    ...
    if (gameMillis - hornetTimer > nextHornetTime) {
      ...
      if (enemyTypeDet > 4) {
        ...
        hornets.add(new Hornet(enemyTypeDet > 1 ? 1 : 0));
        hornetTimer = gameMillis;
      }
    }
    ...
    if (gameMillis - hornetTimer > nextHornetTime) {
      hornets.add(new Hornet(random(forestType) > 4 ? 1 : 0));
      hornetTimer = gameMillis;
    }
  ...
  void showFirefly() {
    ...
    if (forTraining) {
      if (targetable) {
        ...
        noStroke();
        rect(location.x-19.5, location.y-24.5, map(fireflyTap, 1, 2, 39/2, 39.5), 9.5);
        println(fireflyTap);
      }
    }
  }
