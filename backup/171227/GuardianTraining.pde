...
void guardianTrainDisplayRun() {
  ...
  case 5: //Bouncer Guardian
    for (Bee b : trainingBees) b.showBee();
    for (Hornet h : trainingHornets) h.showHornet();
    for (Ladybug lb : trainingLadybugs) lb.showLadybug();
...
void guardianTrainMechanicRun(boolean showText) {
  ...
  case 2: //Hunting Guardian
    ...
    if (trainingHornets.size() < 4) trainingHornets.add(new Hornet((random(1) > 0.6 ? 1 : 0), true));
  ...
  case 4: //Ranged Guardian
    ...
    if (trainingFireflies.size() < 4) {
      trainingFireflies.add(new Firefly(true));
      //trainingFireflies.get(3).updateFireflyHp(10);
    }
    if (trainingHornets.size() < 5) {
      trainingHornets.add(new Hornet((random(1) > 0.6 ? 1 : 0), true));
      //int multiplier = trainingHornets.get(4).getHornetType()+1;
      //trainingHornets.get(4).updateHornetHp(4*multiplier);
    }
    if (pomegranate.size() < 2) pomegranate.add(new ItemSpawn(0));
  ...
  case 5: //Bouncer Guardian
    if (GTObjectiveTime > 0) {
      GTObjectiveTime -= (timeMillis-pMillis);
      GTObjectiveTime = (long)constrain(GTObjectiveTime, 0, 8000);
    } else if (GTObjectiveTime == 0 && trainingHornets.size() == 0) {
      trainingHornets.add(new Hornet(random((GTObjectiveScore+1)) > 0.5045*pow(GTObjectiveScore, 0.6823) ? 1 : 0, true));
      GTObjectiveTime = -1;
    }

    for (Bee b : trainingBees) b.move();
    for (Hornet h : trainingHornets) h.move();
    for (Ladybug lb : trainingLadybugs) lb.move();

    if (GTObjectiveTime < 0) { //round's time is up. reveal everything & spawn hornet

      if (trainingHornets.size() == 0 && trainingBees.size() > 0) { //round success
        ...
        for (int i = 0; i < GTObjectiveScore+1; i++) trainingLadybugs.add(new Ladybug(true));

        GTObjectiveTime = (int)random(6, 8)*1000;
      } else if (trainingBees.size() == 0) { //bee died, round fail
        trainingBees = new ArrayList<Bee>();
        trainingHornets = new ArrayList<Hornet>();
        trainingLadybugs = new ArrayList<Ladybug>();
        ...
        for (int i = 0; i < GTObjectiveScore+1; i++) {
          ...
          //trainingLadybugs.get(i).updateLocation(width/2, height/2);
        }
  ...
  if (roundTime >= upgradeGuardianCost[GTSelected][1]*ROUND_TIME) { //mostly failed. but if the training course has "tier system", then it might not be.
    if (GTObjectiveTier == 0) { //this is a fail
      ...
      if (beesCount < beehiveSize[beehiveTier]) {
        ...
        if (showText) {
          ...
          text("will be delivered in: " + ((5000+trainingFadeOutTimer-gameMillis)/1000) + " seconds", width/2, height/2+110);
        }
      } else {
        textSize(18);
        fill(255, 50, 50);
    ...
    if (beesCount < beehiveSize[beehiveTier]) {
      ...
      if (showText) {
        if (GTSelected == 4) {
          text("A tier " + GTObjectiveTier + ' ' + upgradedGuardianName[GTSelected], width/2, height/2+80);
          text("will be delivered in: " + ((5000+trainingFadeOutTimer-gameMillis)/1000) + " seconds", width/2, height/2+110);
        } else text(upgradedGuardianName[GTSelected] + " will be delivered in: " + ((5000+trainingFadeOutTimer-gameMillis)/1000) + " seconds", width/2, height/2+80);
      }
    } else {
      textSize(18);
      fill(255, 50, 50);
...
int trainingBeesSelectedPrev = 0, trainingBeesSelectedCurr = 0;
    ...
    if (mouseX > gx-25 && mouseX < gx+25 && mouseY > gy-25 && mouseY < gy+25) {
      if (trainingGuardian.getGuardianSelectedStatus() == false) {
        ...
        if (GTSelected == 5) { //bouncer. can select bees & fake bees
          trainingBees.get(0).updateBeeSelectedStatus(false);
          for (Ladybug lb : trainingLadybugs) {
      ...
      case 5: //bouncer guardian
        ...
        trainingBeesSelectedPrev = 0;
        for (Bee b : trainingBees) {
          if (b.getBeeSelectedStatus()) trainingBeesSelectedPrev++;
        }
        for (Ladybug lb : trainingLadybugs) {
          if (lb.getSelectedStatus()) trainingBeesSelectedPrev++;
        }
        trainingBeesSelectedCurr = trainingBeesSelectedPrev;
        for (Bee b : trainingBees) {
        ...
        if (trainingBeesSelectedCurr == 0 && trainingGuardian.getGuardianSelectedStatus() == false && selectedObject == false) { //selected no bee no guardian, but selected a new target, so training guardian go to that location
          float thetaRad = calcMoveTheta(mouseX, mouseY, gx, gy);
          trainingGuardian.updateMoveTheta(thetaRad);

          trainingGuardian.updateGuardianSelectedStatus(false);
          trainingGuardian.updateGuardianMoveTimer(gameMillis);
        } else if (trainingBeesSelectedCurr > 0 && selectedObject == false) { //selected at least 1 bee. move that bee to mouse direction
          if (trainingBees.get(0).getBeeSelectedStatus()) {
            PVector bPos = trainingBees.get(0).getLocation();
            float thetaRad = calcMoveTheta(mouseX, mouseY, bPos.x, bPos.y);
            trainingBees.get(0).updateMoveTheta(thetaRad);

            trainingBees.get(0).updateBeeSelectedStatus(false);
            trainingBees.get(0).updateBeeMoveTimer(gameMillis);
          }
