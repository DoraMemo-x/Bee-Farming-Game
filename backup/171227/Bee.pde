...
static final float[][] upgradeBeeCost = {{80000, 1}, {60000, 1}, {100000, 1}, {80000, 1}}; //money, week
static final String[][] upgradedBeeDescription = {
  {"|b"+upgradedBeeName[0], "|i$"+nfc(int(upgradeBeeCost[0][0])) + ", " + nf(upgradeBeeCost[0][1]) + (upgradeBeeCost[0][1] > 1 ? " weeks" : " week"), "", "|rAttributes:", "- Unable to gather honey.", "- Moves incredibly slowly.", "", "|rAbility "+ char(34) + "" + char(1505)+""+char(1492)+""+char(1503)+""+char(1499)+char(1492)+""+char(1501)+""+char(1496)+""+char(1506)+""+char(1495)+""+char(1506)+""+char(1502)+""+char(1508)+""+char(1499)+"" + char(34)+":", "- Revives A Dead Bee", "  (Reviving takes 30 seconds)"}, //1498=A 
  {"|b"+upgradedBeeName[1], "|i$"+nfc(int(upgradeBeeCost[1][0])) + ", " + nf(upgradeBeeCost[1][1]) + (upgradeBeeCost[1][1] > 1 ? " weeks" : " week"), "", "|rAttribute: Has two lives.", "- First Life:", "  Moves very quickly but gathers honey slowly", "- Second Life:", "  Moves significantly slower but gathers honey much quicker;", "  However, its capacity becomes smaller"}, 
  {"|b"+upgradedBeeName[2], "|i$"+nfc(int(upgradeBeeCost[2][0])) + ", " + nf(upgradeBeeCost[2][1]) + (upgradeBeeCost[2][1] > 1 ? " weeks" : " week"), "", "|rResource Ability " + char(34) + "Nature's Path" + char(34) + ":", "  Stores 20% of its gathered honey,", "  Charges a toggleable ability for", "  planting flowers in the path it moves in", "", "Drawback:", "- Mediocre flight speed"}, 
  {"|b"+upgradedBeeName[3], "|i$"+nfc(int(upgradeBeeCost[3][0])) + ", " + nf(upgradeBeeCost[3][1]) + (upgradeBeeCost[3][1] > 1 ? " weeks" : " week"), "", "|rCooldown Ability " + char(34) + "Adrenaline" + char(34) + ":", "  Increases its flight speed", "  and honey pick-up speed for the duration.", "", "Drawback:", "- Normally mediocre honey pick-up speed"}, 
};

static final int[][] beeCDAVar = {{35000}, {}, {6000, 12000}, {7000, 10000}}; //CDATimer, CDACooldown (note that gardening bee Cooldown is not time, it's honey gram)
//int beeMoveTimer = 0;
int beesCount = 0;
    ...
    if (isAlive && forShop == false) {
      ...
      case 0: //Priest Bee
        if (beeTarget.equals("Revive")) {
          ...
          textSize(10);
          textAlign(CENTER);
          text("On my way", location.x, location.y-15);
        } else if (hyperSelectedStatus) {
          fill(113, 100, 86);
          textSize(10);
          textAlign(CENTER);
          text("Select a dead bee to revive", location.x, location.y-15);
        } else if (CDAbility) { //reviving
          txtChangeTimer -= (timeMillis - pMillis);
          if (txtChangeTimer < 0) {
            appearTxt = "";
          ...
          noStroke();
          fill(255, 255, 0, 150);
          arc(location.x, location.y, diameter*3, diameter*3, -HALF_PI, map(CDATimer, 0, beeCDAVar[0][0], -HALF_PI, PI+HALF_PI), PIE);
      ...
      case 2: //Gardening Bee
        if (CDAbility) {
          ...
          rect(location.x+15.25, map(CDACooldown, beeCDAVar[2][1], 0, location.y+10, location.y-10), location.x+20.75, location.y+9.75);
          rectMode(CORNER);
        }
        break;
  ...
  void move() {
    ...
    if (isAlive) {
      if (beeType >= beeName.length) { //if it is a special bee (trained bee)
        ...
        case 0: //Priest Bee
          ...
          if (CDAbility) { //if using ability
            if (beeSelectedStatus == false) {
              shouldBeeMove = false;
              CDATimer -= (timeMillis-pMillis); //35000ms - timePassed
        ...
        case 2: //Gardening Bee
          if (CDATimer >= beeCDAVar[2][0]) { //if the timer is up
            CDAbility = false;
            CDATimer = 0;
            beeTimeoutMove = true;
            CDACooldown = beeCDAVar[2][1];
          }

          if (CDAbility) {
            CDATimer += (timeMillis-pMillis); //-= timePassed
            ...
            txtChangeTimer += (timeMillis-pMillis); //***** im using txtChangeTimer as the mechanic timer (frequency)
            if (txtChangeTimer >= frequency) { //frequency = once per 500ms (so duration/frequency flowers)

              txtChangeTimer = 0;
              flowers.add(new Flower(location.x, location.y, forestHoneyRng[forestType][0], int(forestHoneyRng[forestType][1]*0.75) ));
              flowers.get(flowers.size()-1).updateRGB(0, 150, 0);
            }
          } else {
            CDACooldown -= honeyIntake;
            honeyIntake = 0;
            CDACooldown = constrain(CDACooldown, 0, beeCDAVar[2][1]);
          }


          print("Requires: "+beeCDAVar[2][1]);
