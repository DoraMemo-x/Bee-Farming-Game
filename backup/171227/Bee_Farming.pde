...
static final String VERSION = "Ver 17.12.25 <Pre-Alpha>";
boolean debug = false;
...
void setup() {
  ...
  showGuardianTrainBtn = new Button("Guardian Training", width-560, height-37.5, 140, 35, 251, 200, 55, 119, 94, 26, false, "default");
...
void draw() {
  ...
  if (gameOver) {
    ...
    if (BTShopScreenActive) {
      ...
      textSize(12);
      textAlign(RIGHT);
      if (BTSelected == -1) text("Demonstration", width/2+200, 100);
      else {
        text("Demonstration - " + upgradedBeeName[BTSelected], width/2+200, 100);
        if (money < upgradeBeeCost[BTSelected][0] || BTPurchased || BTOngoing) BTPurchase.updateGreyOut(true);
        else {
          BTPurchase.updateGreyOut(true);
          for (Bee b : bees) {
            if (b.getBeeName().equals(guardianName[1])) {
              GTPurchase.updateGreyOut(false);
              break; //break out of "for" so it runs faster
            }
          }
  ...
  for (int y = 0; y < 2; y++) {
    for (int x = 0; x < 4; x++) {
      if (resources.size() > counter) {
        resources.get(counter).updateLocation(20 + blockSize/2 + (blockSize+10)*x, 70 + blockSize/2 + (blockSize+10)*y);
        counter++;
      } else break;
    }
  }
...
void mouseReleased() {
  ...
  if (startupScreenActive) {
      ...
      for (int x = 0; x < 5; x++) {
        if (counter+beeInvSlot < inventoryBees.size() && mouseX > 20 + (blockSize+10)*x && mouseX < 20 + (blockSize+10)*x + blockSize && mouseY > 70 + (blockSize+10)*y && mouseY < 70 + (blockSize+10)*y + blockSize) {
          ...
          if (invBType < beeName.length) {
            ...
            if (invB.getIsAlive()) beeInvSell.updateLabel("Sell for $" + nfc((int)upgradeBeeCost[spType][0]/2));
            else beeInvSell.updateLabel("Sell for $" + nfc((int)sqrt(upgradeBeeCost[spType][0]/2)*3));
          }
    ...
    if (guardianInvSell.MouseClicked()) {
    ...
    for (int y = 0; y < 2; y++) {
      for (int x = 0; x < 4; x++) {
        if (counter < resources.size() && mouseX > 20 + (blockSize+10)*x && mouseX < 20 + (blockSize+10)*x + blockSize && mouseY > 70 + (blockSize+10)*y && mouseY < 70 + (blockSize+10)*y + blockSize) {
          itemInvSelected = counter;
          Resource r = resources.get(itemInvSelected);
    ...
    if (itemInvSell.MouseClicked()) {
      money += resourcePrice[resources.get(itemInvSelected).getType()];
      resources.remove(itemInvSelected);
    ...
    if (BTPurchase.MouseClicked()) {
      BTPurchased = true;
      money -= upgradeBeeCost[BTSelected][0];
