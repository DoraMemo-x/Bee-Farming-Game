...
void tutorialDisplay() {
  ...
  if (status == TutorialStatus.BeeSingleMovement) {
    ...
    if (tutorialMinorStep == TutorialStatus.BeeSingleMovement) {
      ...
      if (bees.get(0).getBeeSelectedStatus()) {
        ...
        text("Proceed to the next section.", 200, 50+75+6+10);
        nextPrompt.Draw();
      ...
      for (int i = 0; i < 3; i++) {
        bees.add(new Bee(0));
        bees.get(i).updateHoneyCap(3000);
        bees.get(i).updateLocation(width/2+100, height/2+50);
      }
  ...
  if (status == TutorialStatus.BeeMultipleMovement) {
    if (tutorialMinorStep == TutorialStatus.BeeMultipleMovement) {
      ...
      for (Bee b : bees) {
        ...
        if (b.getCurrHoneyCap() > 3000 || b.getBeeTarget().equals("Flower")) {
  ...
  if (status == TutorialStatus.TopStatsBar) {
    ...
    if (tutorialMinorStep == TutorialStatus.TopStatsBar) {
      ...
      text("the honey will overflow.", 200, 50+75+6+40);
      nextPrompt.Draw();
      ...
      if (roundTime < ROUND_TIME) {
        text("This pie circle shows", 200, 50+75+6-30);
        text("the time passed in the", 200, 50+75+6-10);
        text("current round. Once it is", 200, 50+75+6+10);
        text("full, the round ends.", 200, 50+75+6+30);
      } else {
...
void tutorialClickEvents() {
  ...
  if (status == TutorialStatus.BeeSingleMovement) {
    if (tutorialMinorStep == TutorialStatus.BeeSingleMovement && nextPrompt.MouseClicked()) {  
      ...
      flowers.add(new Flower(width/2+200, height/2+150, 1, 2));
      bees.add(new Bee(0));
      bees.get(0).updateLocation(width/2+100, height/2+50);
    } else if (tutorialMinorStep == TutorialStatus.BeeSingleMovement+0.1) {
      selectionMechanic();

      //change section to next stage code is above.
      //i tried to move it here but everything else fucks up somehow...
    }
  }
  if (status == TutorialStatus.BeeMultipleMovement) {
    if (tutorialMinorStep == TutorialStatus.BeeMultipleMovement) {
