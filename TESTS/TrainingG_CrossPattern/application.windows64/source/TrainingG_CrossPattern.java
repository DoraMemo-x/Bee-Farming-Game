...
public class TrainingG_CrossPattern extends PApplet {
...
public void setup() {
  ...
  noFill();
   //i dont know what effect it has but it looks fancier
  frameRate(60);
...
public void draw() {
  ...
  strokeWeight(1);
  stroke(0);
  ellipse(width/2, height/2, 200, 200); // (x-200)^2 + (y-200)^2 = 100^2
  ...
  if (trainingHornets.size()==1) {
    if (data.size()>0) {
      ...
      textAlign(RIGHT);
      textSize(10);
      text("Impact at: (" + nf(PApplet.parseFloat(data.get(0)), 3, 3) + ", " + nf(PApplet.parseFloat(data.get(1)), 3, 3) + ")", width-10, 10);
      if (PApplet.parseFloat(data.get(2)) > 0) fill(0, 175, 75);
      else if (PApplet.parseFloat(data.get(2)) < 0) fill(255, 100, 100);
      else if (PApplet.parseFloat(data.get(2)) == 0) fill(255, 255, 100);
      else if (PApplet.parseFloat(data.get(2)) == Float.POSITIVE_INFINITY || PApplet.parseFloat(data.get(2)) == Float.NEGATIVE_INFINITY) fill(255, 100, 255);
      text("Gradient: " + data.get(2), width-10, 22);

      fill(125);
      text("Hornet vector: " + nfp(PApplet.parseFloat(data.get(3)), 1, 3) + "i " + nfp(PApplet.parseFloat(data.get(4)), 1, 3) + "j", width-10, 34);
      text("Collision to circle center vector: " + nfp(PApplet.parseFloat(data.get(5)), 1, 3) + "i " + nfp(PApplet.parseFloat(data.get(6)), 1, 3) + "j", width-10, 46);
      text("Vector Theta: " + nf(PApplet.parseFloat(data.get(7)), 2, 3), width-10, 58);
      text("Move Theta: " + nf(PApplet.parseFloat(data.get(8)), 2, 3) + " -> " + nf(PApplet.parseFloat(data.get(10)), 2, 3), width-10, 70);
      if (exception.length() > 1) text("EXCEPTION: " + exception, width-10, 106);

      float _vectorThetaD = PApplet.parseFloat(data.get(7)), _purpleAngleD = PApplet.parseFloat(data.get(9)), _moveThetaD = PApplet.parseFloat(data.get(8));
      if (PApplet.parseFloat(data.get(2)) > 0) {
        float blueAngle = 180 + (90-_vectorThetaD) + 2*_vectorThetaD;
        text("Blue Angle: " + nf(blueAngle, 3, 3), width-10, 82);
        text("Purple Angle: " + nf(PApplet.parseFloat(data.get(9)), 3, 3), width-10, 94);
      }
    }
...
public void mousePressed() {
  if (data.size()>0) {
    tempLinePos[0] = PApplet.parseFloat(data.get(0));
    tempLinePos[1] = PApplet.parseFloat(data.get(1));
    tempLinePos[2] = mouseX;
    tempLinePos[3] = mouseY;
    tempTheta = calcMoveTheta(mouseX, mouseY, PApplet.parseFloat(data.get(0)), PApplet.parseFloat(data.get(1)));
  }
}

public void allCreatureWall(float x1, float y1, float x2, float y2) {
}
...
boolean oneBounce = true, showDebug = false, canMove = true, noLine = false, showTrail = false, showHornet = true;
public void keyPressed() {
  if (key == 'r' && keyPressed) {
    ...
    background(255);
    trainingHornets = new ArrayList<Hornet>();
    for (int i = 0; i < 100; i++) trainingHornets.add(new Hornet(0, true));
...
static final float[] hornetFlightSpeed = {1.25f, 1.5f/*, 2.8*/};
class Hornet {
  int hornetType = -1;
  ...
  Hornet(int _hornetType, boolean _ft) {
    ...
    else location = new PVector(width/2 + random(70)*(random(1)>0.5f?1:-1), height/2 + random(70)*(random(1)>0.5f?1:-1)); 
    if (useManualTheta) moveTheta = radians(manualMoveTheta);
    else moveTheta = radians(random(360));
    locations[0] = location;
  } 

  public void updateSpeed(float _s) {
    speed = _s;
  }
  ...
  public void updateMoveTheta(float _mt) {
    moveTheta = _mt;
  }

  public void showHornet() {
  ...
  public void move() {    
    ...
    if (canMove) {
      ...
      float moveX =   (killFatigue ? 0.6f : 1) * speed*cos(moveTheta);
      float moveY = - (killFatigue ? 0.6f : 1) * speed*sin(moveTheta);

      float hypotenuse = distance_between_two_points(location.x, width/2, location.y, height/2);
      if (hypotenuse > 100-radius) {
        if (bouncing == false || oneBounce == false) {
          ...
          PVector impactPoint = new PVector(location.x+(hypotenuse-(100-radius))*cos(moveTheta), location.y+(hypotenuse-(100-radius))*sin(moveTheta));
          ...
          strokeWeight(1);
          stroke(255, 0, 0);
          if (noLine == false) line(width/2, height/2, impactPoint.x, impactPoint.y);
          linePos[0] = width/2;
          linePos[1] = height/2;
          linePos[2] = impactPoint.x;
          linePos[3] = impactPoint.y;

          float m = 1/((location.y-height/2)/(location.x-width/2));
          data.add(str(m));
          stroke(0, 255, 0);
          ...
          PVector b = new PVector(impactPoint.x-width/2, impactPoint.y-height/2);
          a.normalize();
          b.normalize();
          ...
          if (m > 0) {
            ...
            if (abs(moveThetaD - newMoveThetaD) < 0.5f) { //(+), EXCEPTION: PIERCES THROUGH
            ...
            float detX = width/2 - location.x; //center - loc
            float detY = height/2 - location.y; //center - loc
            //strokeWeight(5);
            //point(impactPoint.x, impactPoint.y);
            ...
            if (detX < 0 && detY > 0) { // - +, quad I
              if (moveThetaD > 90 && moveThetaD <= 180) { // 90 - 180 (included 180), -2i
                ...
                float redLineAngleD = degrees( abs(atan2( (impactPoint.y-height/2), (impactPoint.x-width/2) )) ) + 180; //+180, opposite dir
              ...
              if (moveThetaD > 270 && moveThetaD <= 360) { // 270 - 360 (included 360), -2i
                ...
                //determine which direction is the line coming from
                //"dis is the direction when u stand at the point of collision looking out in the direction of the normal btw."
                float redLineAngleD = 180 - degrees( abs(atan2( (impactPoint.y-height/2), (impactPoint.x-width/2) )) ); //atan2 calculation is weird. 180 - instead.
          ...
          if (vectorThetaD > 2.5f && abs(abs(moveTheta-radians(moveThetaD))-PI) < radians(1)) { //EXCEPTION: self reflection
          ...
          moveX =   (killFatigue ? 0.6f : 1) * 3*speed*cos(moveTheta);
          moveY = - (killFatigue ? 0.6f : 1) * 3*speed*sin(moveTheta);
        }
      } else bouncing = false;
  ...
  public int getHornetType() {
    return hornetType;
  }
}

public float y(float _m, float _x, float _x1, float _y1) {
  float __m = _m, __x = _x, __x1 = _x1, __y1 = _y1;
  float result = __m*(__x-__x1) + __y1;
  return result;
}
public float x(float _m, float _x1, float _y, float _y1) {
  float __m = _m, __x1 = _x1, __y = _y, __y1 = _y1;
  float result = (__y-__y1)/__m + __x1;
  return result;
}

static final float[] guardianFlightSpeed = {1.32f, 1.65f, 1.825f, 1.65f, 1.4f, 1.5f, 2.5f, 1.5f};
...
class TrainingGuardian {
  ...
  public void updateShouldGuardianMove(boolean _sgm) {
    shouldGuardianMove = _sgm;
  }

  public void updateMoveTheta(float _theta) {
    moveTheta = _theta;
  }

  public void updateGuardianTimeoutMove(boolean newGuardianTimeoutMove) {
    guardianTimeoutMove = newGuardianTimeoutMove;
  }

  public void updateGuardianMoveTimer(long newGuardianMoveTimer) {
    guardianMoveTimer = newGuardianMoveTimer;
  }

  public void updateGuardianSelectedStatus(boolean _guardianSelectedStatus) {
    guardianSelectedStatus = _guardianSelectedStatus;
  }

  public void updateGuardianTarget(String _guardianTarget) {
  ...
  public boolean getGuardianSelectedStatus() {
...
public float distance_between_two_points(float _x1, float _x2, float _y1, float _y2) {
  float result = sqrt(pow((_x1-_x2), 2) + pow((_y1-_y2), 2));  
  return result;
}
public float returnRealTheta(float __theta, float determinantX, float determinantY) {
...
public float calcMoveTheta(float targetX, float targetY, float objectX, float objectY) {
...
public void polygon(float x, float y, float radius, int npoints) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "TrainingG_CrossPattern" };
    if (passedArgs != null) {
